...
function template_header($name='',$host='',$port='',$desc='') {
	print <<<EOT
...
<input type="hidden" name="op" value="list"  value="{$_GET['list']}"/>
...
if (array_key_exists('op',$_GET)) {
	...
	elseif ($_GET['op'] == 'list') {
		...
		$port_s = return_has_key($_GET,'port',true);
		$port = intval($port_s);
		...
		template_header($name,$host,$port_s,$desc);
